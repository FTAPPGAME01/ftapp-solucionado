...
io.on('connection', (socket) => {
    ...
    socket.on('getLatestState', () => {
        if (latestGameState) {
            socket.emit('latestState', latestGameState);
        }
    });

    socket.on('saveState', (state) => {
        if (!latestGameState || state.timestamp > latestGameState.timestamp) {
            latestGameState = state;
            socket.broadcast.emit('latestState', state);
        }
    });
